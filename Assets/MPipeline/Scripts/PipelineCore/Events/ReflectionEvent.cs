...
namespace MPipeline
{
    ...
    public unsafe sealed class ReflectionEvent : PipelineEvent
    {
        ...
        public override void FrameUpdate(PipelineCamera cam, ref PipelineCommandData data)
        {
            ...
            buffer.SetRenderTarget(color: cam.targets.renderTargetIdentifier, depth: ShaderIDs._DepthBufferTexture);
            buffer.DrawMesh(GraphicsUtility.mesh, Matrix4x4.identity, reflectionMat, 0, 3);
            int releaseSSRID = -1;
            if (ssrEvents.enabled && !RenderPipeline.renderingEditor)
            {

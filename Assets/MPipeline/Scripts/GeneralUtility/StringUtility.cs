...
public unsafe struct MStringBuilder
{
    ...
    public void Add(char tar)
    {
        int lastLength = str.Length;
        int targetLength = str.Length + 1;
    ...
    public void Add(char* chrs, int len)
    {
        int lastLength = str.Length;
        int targetLength = str.Length + len;
        if (targetLength > capacity)
        {
            capacity = targetLength;
            string lastStr = str;
        ...
        {
            char* ptr = dest + lastLength;
            UnsafeUtility.MemCpy(ptr, chrs, sizeof(char) * len);
        }
    }

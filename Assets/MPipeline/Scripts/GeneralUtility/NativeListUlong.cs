...
public unsafe struct NativeList_ulong : IEnumerable<ulong>
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public NativeList_ulong(int count, int capacity, Allocator alloc)
    {
        isCreated = true;
        data = MUnsafeUtility.Malloc<NativeListData>(sizeof(NativeListData), alloc);
        data->count = count;
    ...
    public void RemoveElement(ulong target, System.Func<ulong, ulong, bool> conditionFunc)
    {
    ...
    public ulong* unsafePtr
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get
        {
            return (ulong*)data->ptr;
        }
    }
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Dispose()
    ...
    public ref ulong this[int id]
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get
        {
            ulong* ptr = (ulong*)data->ptr;
            return ref *(ptr + id);
        }
    }

    public ref ulong this[uint id]
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get
        {
    ...
    public void AddRange(ulong[] array)
    {
        ...
        fixed (void* source = &array[0])
        {
            ...
            UnsafeUtility.MemCpy(dest, source, array.Length * sizeof(ulong));
        }
    }
    ...
    public void AddRange(ulong* array, int length)
    {
        ...
        ResizeToCount();
        void* dest = unsafePtr + last;
        UnsafeUtility.MemCpy(dest, array, length * sizeof(ulong));
    ...
    public int ConcurrentAdd(ulong value)
    {
    ...
    public int ConcurrentAdd(ref ulong value)
    {
        int last = Interlocked.Increment(ref data->count);
        //Concurrent Resize
        if (last <= data->capacity)
    ...
    public int ConcurrentAdd(ulong value, object lockerObj)
    {
        int last = Interlocked.Increment(ref data->count);
        //Concurrent Resize
        if (last > data->capacity)
        {
            lock (lockerObj)
            {
                if (last > data->capacity)
                {
                    ...
                    void* newPtr = MUnsafeUtility.Malloc<ulong>(sizeof(ulong) * newCapacity, data->allocator);
                    UnsafeUtility.MemCpy(newPtr, data->ptr, sizeof(ulong) * data->count);
                    UnsafeUtility.Free(data->ptr, data->allocator);
                    data->ptr = newPtr;
    ...
    public int ConcurrentAdd(ref ulong value, object lockerObj)
    {
        int last = Interlocked.Increment(ref data->count);
        //Concurrent Resize
        if (last > data->capacity)
    ...
    {
        return new ListIenumerator_ulong(data);
    }
}

public unsafe struct ListIenumerator_ulong : IEnumerator<ulong>
{
    [NativeDisableUnsafePtrRestriction]
    private NativeListData* data;
    private int iteIndex;
    public ListIenumerator_ulong(NativeListData* dataPtr)
    {
        data = dataPtr;
        iteIndex = -1;
    }
    object IEnumerator.Current
    {
        get
        {
            return ((ulong*)data->ptr)[iteIndex];
        }
    }

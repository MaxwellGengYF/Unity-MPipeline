...
sampler2D _SSR_RayCastRT, _SSR_GetSSRColor_RT, _SSR_TemporalPrev_RT, _SSR_TemporalCurr_RT, _CameraDepthTexture, _CameraMotionVectorsTexture, _CameraGBufferTexture1, _CameraGBufferTexture2;
...
float4 ScreenSpaceReflection_HiZTrace(PixelInput i) : SV_Target
{
	...
	ViewPos /= ViewPos.w;*/
	float2 UV = i.uv.xy;
	float SceneDepth = tex2D(_CameraDepthTexture, UV).x;
	...
	float3 rayDir = normalize( (rayProj.xyz / max(1e-4, rayProj.w)) - ScreenPos);
	rayDir.xy *= 0.5;
...
float4 ScreenSpaceReflection_GetColor(PixelInput i) : SV_Target
{
	...
	float SceneDepth = GetDepth(_CameraDepthTexture, UV);
